    ...
    <footer className="bg-navy-800 text-white">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-4">
          <div>
            <h3 className="font-heading text-lg font-bold text-gold-500">Your Next Opportunity Starts Here</h3>
            ...
            <Link
              to="/contact"
              className="mt-5 inline-flex items-center rounded-md bg-gold-500 px-5 py-2.5 text-sm font-bold uppercase tracking-wide text-navy-800 transition hover:bg-gold-400"
            >
              Let's Connect →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 lg:col-span-2 lg:grid-cols-4">
            {STATS.map((stat) => (
              <div key={stat.label} className="border-l-2 border-gold-500/60 pl-3">
                <div className="font-heading text-2xl font-bold text-gold-500">{stat.value}</div>
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm text-navy-100">
              <li className="flex items-center gap-2">
                ...
                <a href={CONTACT.phoneHref} className="hover:text-gold-400">{CONTACT.phoneDisplay}</a>
              </li>
              <li className="flex items-center gap-2">
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-6 border-t border-white/10 pt-8 sm:flex-row">
          ...
          <nav className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {NAV_ITEMS.map((item) => (
              <Link key={item.to} to={item.to} className="text-xs font-semibold uppercase tracking-wide text-navy-100 hover:text-gold-400">
                {item.label}
              </Link>
            ))}
          </nav>
